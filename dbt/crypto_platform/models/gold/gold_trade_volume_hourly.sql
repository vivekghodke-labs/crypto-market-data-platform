...
hourly_trades as (
    ...
    select
        ...
        date_trunc('hour', trade_timestamp)         as trade_hour,
...
aggregated as (
    ...
    select
        ...
        (sum(price * quantity) 
        / NULLIF(sum(quantity), 0))                 as vwap_hourly,
        ...
        -- Average trade size
        (sum(quantity) / NULLIF(count(*), 0))       as avg_trade_size_btc,
        (sum(price * quantity) 
        / NULLIF(count(*), 0))                      as avg_trade_size_usd
...
final as (
    ...
    select
        ...
        strftime(trade_hour, '%A')                  as day_of_week,
